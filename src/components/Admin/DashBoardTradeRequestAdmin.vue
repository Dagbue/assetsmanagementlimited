<template>
  <div class="alpha">
    ...
    <delete-trade-request-modal @close="hideDialog" v-if="dialogIsVisible" :selected-item="selectedItem"/>
...
import DeleteTradeRequestModal from "@/components/baseComponents/modal/DeleteTradeRequestModal.vue";


export default {
  name: "DashBoardTradeRequestAdmin",
  components: {DeleteTradeRequestModal, BaseLoader2},
  data () {
    return {
      SelectEmail: "",
      history: [],
  ...
  methods: {
    ...
    async showDialog(child) {
      this.selectedItem = child.tradeId;
